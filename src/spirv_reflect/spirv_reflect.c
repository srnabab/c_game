...
#define SPV_REFLECT_ASSERT(COND) SDL_assert(COND);
#else
...
static uint32_t RoundUp(uint32_t value, uint32_t multiple) {
  SDL_assert(multiple && ((multiple & (multiple - 1)) == 0));
...
static SpvReflectResult IntersectSortedAccessedVariable(const SpvReflectPrvAccessedVariable* p_arr0, size_t arr0_size,
  ...
  *res_size = 0;
  ...
  if (*res_size > 0) {
    *pp_res = (uint32_t*)SDL_calloc(*res_size, sizeof(**pp_res));
    if (IsNull(*pp_res)) {
...
static SpvReflectResult ReadU32(SpvReflectPrvParser* p_parser, uint32_t word_offset, uint32_t* p_value) {
  SDL_assert(IsNotNull(p_parser));
  SDL_assert(IsNotNull(p_parser->spirv_code));
  SDL_assert(InRange(p_parser, word_offset));
...
static SpvReflectResult ReadStr(SpvReflectPrvParser* p_parser, uint32_t word_offset, uint32_t word_index, uint32_t word_count,
  ...
  SDL_assert(IsNotNull(p_parser));
  SDL_assert(IsNotNull(p_parser->spirv_code));
  SDL_assert(InRange(p_parser, limit));
  SpvReflectResult result = SPV_REFLECT_RESULT_ERROR_SPIRV_UNEXPECTED_EOF;
...
static SpvReflectBlockVariable* GetRefBlkVar(SpvReflectPrvParser* p_parser, SpvReflectPrvAccessChain* p_access_chain) {
  uint32_t base_id = p_access_chain->base_id;
  SpvReflectPrvNode* base_node = FindNode(p_parser, base_id);
  SDL_assert(base_node->op == SpvOpLoad);
  UNCHECKED_READU32(p_parser, base_node->word_offset + 3, base_id);
  SpvReflectPrvAccessChain* base_ac = FindAccessChain(p_parser, base_id);
  SDL_assert(base_ac != 0);
  SpvReflectBlockVariable* base_var = base_ac->block_var;
  SDL_assert(base_var != 0);
  return base_var;
}
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser* p_parser) {
  ...
  p_parser->nodes = (SpvReflectPrvNode*)SDL_calloc(p_parser->node_count, sizeof(*(p_parser->nodes)));
  if (IsNull(p_parser->nodes)) {
    return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
  }
  ...
  if (p_parser->access_chain_count > 0) {
    p_parser->access_chains = (SpvReflectPrvAccessChain*)SDL_calloc(p_parser->access_chain_count, sizeof(*(p_parser->access_chains)));
    ...
    switch (p_node->op) {
      ...
      case SpvOpSource: {
        ...
        if (p_node->word_count >= 5) {
          ...
          char* p_source_temp = (char*)SDL_calloc(source_len + 1, sizeof(char));
      ...
      case SpvOpSourceContinued: {
        ...
        char* p_continued_source = (char*)SDL_calloc(source_len + embedded_source_len + 1, sizeof(char));
      ...
      case SpvOpAccessChain: {
        ...
        if (p_access_chain->index_count > 0) {
          p_access_chain->indexes = (uint32_t*)SDL_calloc(p_access_chain->index_count, sizeof(*(p_access_chain->indexes)));
          ...
          for (uint32_t index_index = 0; index_index < p_access_chain->index_count; ++index_index) {
            ...
            if ((p_index_value_node != NULL) &&
              ...
              SDL_assert(index_value != UINT32_MAX);
...
static SpvReflectResult ParseStrings(SpvReflectPrvParser* p_parser) {
  SDL_assert(IsNotNull(p_parser));
  SDL_assert(IsNotNull(p_parser->spirv_code));
  SDL_assert(IsNotNull(p_parser->nodes));
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    p_parser->strings = (SpvReflectPrvString*)SDL_calloc(p_parser->string_count, sizeof(*(p_parser->strings)));
    ...
    for (size_t i = 0; i < p_parser->node_count; ++i) {
      ...
      SDL_assert(string_index < p_parser->string_count);
...
static SpvReflectResult ParseSource(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code)) {
    ...
    if (IsNotNull(p_parser->source_embedded)) {
      ...
      char* p_source = (char*)SDL_calloc(source_len + 1, sizeof(char));
...
static SpvReflectResult ParseFunction(SpvReflectPrvParser* p_parser, SpvReflectPrvNode* p_func_node, SpvReflectPrvFunction* p_func,
  ...
  if (p_func->parameter_count > 0) {
    p_func->parameters = (uint32_t*)SDL_calloc(p_func->parameter_count, sizeof(*(p_func->parameters)));
  ...
  if (p_func->callee_count > 0) {
    p_func->callees = (uint32_t*)SDL_calloc(p_func->callee_count, sizeof(*(p_func->callees)));
  ...
  if (p_func->accessed_variable_count > 0) {
    p_func->accessed_variables =
        (SpvReflectPrvAccessedVariable*)SDL_calloc(p_func->accessed_variable_count, sizeof(*(p_func->accessed_variables)));
  ...
  if (p_func->callee_count > 0) {
    SDL_qsort(p_func->callees, p_func->callee_count, sizeof(*(p_func->callees)), SortCompareUint32);
  }
  p_func->callee_count = (uint32_t)DedupSortedUint32(p_func->callees, p_func->callee_count);

  if (p_func->accessed_variable_count > 0) {
    SDL_qsort(p_func->accessed_variables, p_func->accessed_variable_count, sizeof(*(p_func->accessed_variables)),
...
static SpvReflectResult ParseFunctions(SpvReflectPrvParser* p_parser) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    p_parser->functions = (SpvReflectPrvFunction*)SDL_calloc(p_parser->function_count, sizeof(*(p_parser->functions)));
    ...
    SDL_qsort(p_parser->functions, p_parser->function_count, sizeof(*(p_parser->functions)), SortCompareFunctions);
    ...
    for (size_t i = 0; i < p_parser->function_count; ++i) {
      ...
      p_func->callee_ptrs = (SpvReflectPrvFunction**)SDL_calloc(p_func->callee_count, sizeof(*(p_func->callee_ptrs)));
...
static SpvReflectResult ParseMemberCounts(SpvReflectPrvParser* p_parser) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    for (uint32_t i = 0; i < p_parser->node_count; ++i) {
      ...
      p_node->member_names = (const char**)SDL_calloc(p_node->member_count, sizeof(*(p_node->member_names)));
      if (IsNull(p_node->member_names)) {
        return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
      }

      p_node->member_decorations = (SpvReflectPrvDecorations*)SDL_calloc(p_node->member_count, sizeof(*(p_node->member_decorations)));
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  if (spec_constant_count > 0) {
    p_module->spec_constants = (SpvReflectSpecializationConstant*)SDL_calloc(spec_constant_count, sizeof(*p_module->spec_constants));
...
static SpvReflectResult EnumerateAllUniforms(SpvReflectShaderModule* p_module, size_t* p_uniform_count, uint32_t** pp_uniforms) {
  ...
  *pp_uniforms = (uint32_t*)SDL_calloc(*p_uniform_count, sizeof(**pp_uniforms));
  ...
  SDL_qsort(*pp_uniforms, *p_uniform_count, sizeof(**pp_uniforms), SortCompareUint32);
  return SPV_REFLECT_RESULT_SUCCESS;
}
...
static SpvReflectResult ParseType(SpvReflectPrvParser* p_parser, SpvReflectPrvNode* p_node,
  ...
  if (p_node->member_count > 0) {
    ...
    p_type->members = (SpvReflectTypeDescription*)SDL_calloc(p_type->member_count, sizeof(*(p_type->members)));
    ...
    switch (p_node->op) {
      ...
      case SpvOpTypeStruct: {
        ...
        for (; word_index < p_node->word_count; ++word_index, ++member_index) {
          ...
          SDL_assert(member_index < p_type->member_count);
          // Parse member type
...
static SpvReflectResult ParseTypes(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->_internal->type_descriptions = (SpvReflectTypeDescription*)SDL_calloc(p_module->_internal->type_description_count,
  ...
  if (p_parser->physical_pointer_struct_count > 0) {
    p_parser->physical_pointer_structs = (SpvReflectPrvPhysicalPointerStruct*)SDL_calloc(p_parser->physical_pointer_struct_count,
...
static SpvReflectResult ParseCapabilities(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->capabilities = (SpvReflectCapability*)SDL_calloc(p_module->capability_count, sizeof(*(p_module->capabilities)));
...
static int SortCompareDescriptorBinding(const void* a, const void* b) {
  ...
  if (value == 0) {
    ...
    // unique.
    SDL_assert(p_elem_a->spirv_id != p_elem_b->spirv_id);
...
static SpvReflectResult ParseDescriptorBindings(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->descriptor_bindings =
      (SpvReflectDescriptorBinding*)SDL_calloc(p_module->descriptor_binding_count, sizeof(*(p_module->descriptor_bindings)));
  ...
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    ...
    if (p_type->op == SpvOpTypePointer) {
      SDL_assert(p_type->storage_class != -1 && "Pointer types must have a valid storage class.");
  ...
  if (p_module->descriptor_binding_count > 0) {
    SDL_qsort(p_module->descriptor_bindings, p_module->descriptor_binding_count, sizeof(*(p_module->descriptor_bindings)),
  ...
  for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count; ++descriptor_index) {
    ...
    if ((int)p_descriptor->descriptor_type == (int)INVALID_VALUE) {
      switch (p_type->type_flags & SPV_REFLECT_TYPE_FLAG_EXTERNAL_MASK) {
        default:
          SDL_assert(false && "unknown type flag");
          break;
        ...
        case SPV_REFLECT_TYPE_FLAG_EXTERNAL_IMAGE: {
          if (p_descriptor->image.dim == SpvDimBuffer) {
            switch (p_descriptor->image.sampled) {
              default:
                SDL_assert(false && "unknown texel buffer sampled value");
                break;
              case IMAGE_SAMPLED:
            ...
            switch (p_descriptor->image.sampled) {
              default:
                SDL_assert(false && "unknown image sampled value");
                break;
              case IMAGE_SAMPLED:
        ...
        case SPV_REFLECT_TYPE_FLAG_EXTERNAL_BLOCK: {
          if (p_type->decoration_flags & SPV_REFLECT_DECORATION_BLOCK) {
            ...
            SDL_assert(false && "unknown struct");
          }
        } break;
...
static SpvReflectResult ParseDescriptorBlockVariable(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  if (IsNotNull(p_type->members) && (p_type->member_count > 0)) {
    ...
    p_var->members = (SpvReflectBlockVariable*)SDL_calloc(p_var->member_count, sizeof(*p_var->members));
...
static SpvReflectResult ParseDescriptorBlockVariableSizes(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  SpvReflectBlockVariable** pp_member_offset_order =
      (SpvReflectBlockVariable**)SDL_calloc(p_var->member_count, sizeof(SpvReflectBlockVariable*));
...
static SpvReflectResult ParseDescriptorBlockVariableUsage(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  switch (op_type) {
    ...
    case SpvOpTypeStruct: {
      SDL_assert(p_var->member_count > 0);
      if (p_var->member_count == 0) {
...
static SpvReflectResult ParseInterfaceVariable(SpvReflectPrvParser* p_parser,
  ...
  if (p_type->member_count > 0) {
    p_var->member_count = p_type->member_count;
    p_var->members = (SpvReflectInterfaceVariable*)SDL_calloc(p_var->member_count, sizeof(*p_var->members));
    if (IsNull(p_var->members)) {
      return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
    }
...
static SpvReflectResult ParseInterfaceVariables(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  if (p_entry->input_variable_count > 0) {
    p_entry->input_variables =
        (SpvReflectInterfaceVariable**)SDL_calloc(p_entry->input_variable_count, sizeof(*(p_entry->input_variables)));
  ...
  if (p_entry->output_variable_count > 0) {
    p_entry->output_variables =
        (SpvReflectInterfaceVariable**)SDL_calloc(p_entry->output_variable_count, sizeof(*(p_entry->output_variables)));
  ...
  if (p_entry->interface_variable_count > 0) {
    p_entry->interface_variables =
        (SpvReflectInterfaceVariable*)SDL_calloc(p_entry->interface_variable_count, sizeof(*(p_entry->interface_variables)));
...
static SpvReflectResult EnumerateAllPushConstants(SpvReflectShaderModule* p_module, size_t* p_push_constant_count,
  ...
  *p_push_constants = (uint32_t*)SDL_calloc(*p_push_constant_count, sizeof(**p_push_constants));
  ...
  SDL_qsort(*p_push_constants, *p_push_constant_count, sizeof(**p_push_constants), SortCompareUint32);
  return SPV_REFLECT_RESULT_SUCCESS;
}
...
static SpvReflectResult ParseFunctionParameterAccess(SpvReflectPrvParser* p_parser, uint32_t callee_function_id,
  ...
  SDL_assert(function_parameter_index < p_func->parameter_count);
...
static SpvReflectResult ParseStaticallyUsedResources(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  if (called_function_count > 0) {
    p_called_functions = (uint32_t*)SDL_calloc(called_function_count, sizeof(*p_called_functions));
  ...
  if (called_function_count > 0) {
    SDL_qsort(p_called_functions, called_function_count, sizeof(*p_called_functions), SortCompareUint32);
  }
  ...
  if (used_acessed_count > 0) {
    p_used_accesses = (SpvReflectPrvAccessedVariable*)SDL_calloc(used_acessed_count, sizeof(SpvReflectPrvAccessedVariable));
  ...
  if (used_acessed_count > 0) {
    SDL_qsort(p_used_accesses, used_acessed_count, sizeof(*p_used_accesses), SortCompareAccessedVariable);
  }
    ...
    if (byte_address_buffer_offset_count > 0) {
      ...
      if (multi_entrypoint) {
        ...
        p_binding->byte_address_buffer_offsets =
            (uint32_t*)SDL_calloc(byte_address_buffer_offset_count + p_binding->byte_address_buffer_offset_count, sizeof(uint32_t));
        ...
        p_binding->byte_address_buffer_offsets = (uint32_t*)SDL_calloc(byte_address_buffer_offset_count, sizeof(uint32_t));
      }
      ...
      if (multi_entrypoint) {
        SDL_qsort(p_binding->byte_address_buffer_offsets, p_binding->byte_address_buffer_offset_count,
...
static SpvReflectResult ParseEntryPoints(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->entry_points = (SpvReflectEntryPoint*)SDL_calloc(p_module->entry_point_count, sizeof(*(p_module->entry_points)));
  ...
  for (size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i) {
    ...
    if (interface_variable_count > 0) {
      p_interface_variables = (uint32_t*)SDL_calloc(interface_variable_count, sizeof(*(p_interface_variables)));
...
static SpvReflectResult ParseExecutionModes(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    uint32_t* indices = (uint32_t*)SDL_calloc(p_module->entry_point_count, sizeof(indices));
    if (IsNull(indices)) {
      return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
    }
    for (size_t entry_point_idx = 0; entry_point_idx < p_module->entry_point_count; ++entry_point_idx) {
      ...
      if (p_entry_point->execution_mode_count > 0) {
        p_entry_point->execution_modes =
            (SpvExecutionMode*)SDL_calloc(p_entry_point->execution_mode_count, sizeof(*p_entry_point->execution_modes));
...
static SpvReflectResult ParsePushConstantBlocks(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->push_constant_blocks =
      (SpvReflectBlockVariable*)SDL_calloc(p_module->push_constant_block_count, sizeof(*p_module->push_constant_blocks));
...
static SpvReflectResult ParseEntrypointDescriptorSets(SpvReflectShaderModule* p_module) {
  ...
  for (uint32_t i = 0; i < p_module->entry_point_count; ++i) {
    ...
    if (p_entry->descriptor_set_count > 0) {
      p_entry->descriptor_sets = (SpvReflectDescriptorSet*)SDL_calloc(p_entry->descriptor_set_count, sizeof(*p_entry->descriptor_sets));
    ...
    for (uint32_t j = 0; j < p_module->descriptor_set_count; ++j) {
      ...
      p_entry_set->bindings = (SpvReflectDescriptorBinding**)SDL_calloc(count, sizeof(*p_entry_set->bindings));
...
static SpvReflectResult ParseDescriptorSets(SpvReflectShaderModule* p_module) {
  ...
  if (p_module->descriptor_set_count > 0) {
    SDL_qsort(p_module->descriptor_sets, p_module->descriptor_set_count, sizeof(*(p_module->descriptor_sets)),
  ...
  for (uint32_t i = 0; i < p_module->descriptor_set_count; ++i) {
    ...
    p_set->bindings = (SpvReflectDescriptorBinding**)SDL_calloc(p_set->binding_count, sizeof(*(p_set->bindings)));
    ...
    for (uint32_t j = 0; j < p_module->descriptor_binding_count; ++j) {
      ...
      if (p_descriptor->set == p_set->set) {
        SDL_assert(descriptor_index < p_set->binding_count);
...
static SpvReflectResult SynchronizeDescriptorSets(SpvReflectShaderModule* p_module) {
  // SDL_Free and reset all descriptor set numbers
  for (uint32_t i = 0; i < SPV_REFLECT_MAX_DESCRIPTOR_SETS; ++i) {
    SpvReflectDescriptorSet* p_set = &p_module->descriptor_sets[i];
...
static SpvReflectResult CreateShaderModule(uint32_t flags, size_t size, const void* p_code, SpvReflectShaderModule* p_module) {
...
#ifdef __cplusplus
  p_module->_internal = (SpvReflectShaderModule::Internal*)SDL_calloc(1, sizeof(*(p_module->_internal)));
#else
  p_module->_internal = SDL_calloc(1, sizeof(*(p_module->_internal)));
#endif
  ...
  if (flags & SPV_REFLECT_MODULE_FLAG_NO_COPY) {
    ...
    p_module->_internal->spirv_size = size;
    p_module->_internal->spirv_code = (uint32_t*)SDL_calloc(1, p_module->_internal->spirv_size);
    p_module->_internal->spirv_word_count = (uint32_t)(size / SPIRV_WORD_SIZE);
...
static void SafeFreeBlockVariables(SpvReflectBlockVariable* p_block) {
  ...
  // We share pointers to Physical Pointer structs and don't want to double SDL_free
...
void spvReflectDestroyShaderModule(SpvReflectShaderModule* p_module) {
  ...
  // SDL_Free SPIR-V code if there was a copy
